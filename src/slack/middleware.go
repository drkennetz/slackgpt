...
func middlewareAppMentionEvent(evt *socketmode.Event, client *socketmode.Client,
	gptClient gpt3.Client, ctx context.Context, logger *log.Logger) {
...
func middlewareMessageEvent(evt *socketmode.Event, client *socketmode.Client, gptClient gpt3.Client, ctx context.Context, logger *log.Logger) {
	...
	if ev.BotID != "" {
		logger.Println("The bot will answer itself.")
		return
	}
	question := ev.Text
	gpt3Resp, err := chatgpt.GetStringResponse(gptClient, ctx, question)
